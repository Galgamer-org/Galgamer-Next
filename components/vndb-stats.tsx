...
export default async function VndbStats({ vndbId }: { vndbId: string }) {
    ...
    const getReleasecInfo = async (name: String) => {
    ...
    const length_time = function (min) {
        return ((min / 60) + "小时 " + (min % 60) + "min")
    }

    const languages_flag = function (languages,language) {
        let result = null;
    ...
    if (vndbId !== null && getBasicInfo(vndbId) != null) {
        ...
        releaseInfo = await getReleasecInfo(basicInfo.title);
    ...
    return (
        <div className={cn(style.vndbWrapper, "shadow")}>
            ...
            <Row className={style.dataContainer}>
                <Col className={cn(style.rating, "col-12 col-md-6")}>評分和投票</Col>
